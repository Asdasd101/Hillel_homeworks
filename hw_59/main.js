...
async function compressFile(filePath) {
  try {
    ...
    while (true) {
      if (counter === 0) {
        targetPath = path.join(parsedPath.dir, `${parsedPath.name}${parsedPath.ext}.gz`);
      } else {
        targetPath = path.join(parsedPath.dir, `${parsedPath.name}(${counter})${parsedPath.ext}.gz`);
      }
    ...
    throw new Error(`Компресія провалилася: ${error.message}`);
  }
}
...
async function decompressFile(compressedFilePath, destinationFilePath) {
  try {
    ...
    throw new Error(`Декомпресія провалилася: ${error.message}`);
  }
}
